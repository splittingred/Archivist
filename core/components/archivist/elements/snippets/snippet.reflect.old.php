...
//  Includes
// ---------------------------------------------------
$reflect_base = $modx->getOption('reflect_base',$scriptProperties,$modx->getOption('reflect.core_path',null,$modx->getOption('core_path').'components/reflect/'));
/*
    Param: ditto_base
...
include $reflect_base."configs/default.config.php";
include $reflect_base."default.templates.php";
if ($config != "default") {
    include (substr($config, 0, 5) != "@FILE") ? $reflect_base."configs/$config.config.php" : $modx->config['base_path'].trim(substr($config, 5));
}
...
/*
    ...
    1 - on
*/
$targetID = isset($targetID) ? $targetID : $modx->resource->id;
/*
    Param: targetID
        ...
        foreach ($months as $mon=>$month) {
            ...
            if ($showItems) {
                foreach ($month as $item) {
                    $items[$year][$mon]['items'][] = $ditto->render($item, $templates['item'], false, $templatesDateSource, $dateFormat, array());
                }
